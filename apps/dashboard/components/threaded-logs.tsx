...
const LOG_STYLES: Record<string, { color: string; prefix: string }> = {
  info:    { color: "text-slate-700",   prefix: "›" },
  success: { color: "text-emerald-600", prefix: "✓" },
  warning: { color: "text-amber-600",   prefix: "⚠" },
...
const AGENT_COLORS: Record<string, string> = {
  ...
  backend_dev:        "bg-emerald-500/20 text-emerald-700 border-emerald-500/30",
  frontend_dev:       "bg-amber-500/20 text-amber-700 border-amber-500/30",
  qa_engineer:        "bg-pink-500/20 text-pink-300 border-pink-500/30",
  devops_engineer:    "bg-slate-500/20 text-slate-700 border-slate-500/30",
  tech_writer:        "bg-yellow-500/20 text-yellow-300 border-yellow-500/30",
  system:             "bg-blue-500/20 text-blue-300 border-blue-500/30",
  root:               "bg-slate-500/20 text-slate-700 border-slate-400/30",
};

function getAgentColor(agentId: string, agents: AgentNode[]): string {
  const node = agents.find((a) => a.id === agentId);
  if (node) return AGENT_COLORS[node.role] ?? "bg-slate-700/40 text-slate-500 border-slate-600/30";
...
function LogEntry({ log, agents }: { log: SessionLog; agents: AgentNode[] }) {
  ...
  return (
    <div className="flex gap-2 group hover:bg-slate-100/30 rounded px-1 py-0.5 transition-colors">
      {/* Timestamp */}
  ...
  return (
    <div className="border border-slate-200 rounded-lg overflow-hidden">
      ...
      >
        ...
        <Badge variant="outline" className="text-[10px] h-4 px-1 border-slate-200 text-slate-500 shrink-0">
          {logs.length}
        </Badge>
      ...
      {/* Thread body */}
      {isExpanded && (
        <div className="p-2 font-mono text-xs space-y-0.5 bg-slate-50/40 border-t border-slate-200">
          {logs.map((log) => (
  ...
  return (
    <div className={cn("flex flex-col h-full", className)}>
      ...
      <div className="flex items-center gap-2 px-4 py-2 bg-white border-b border-slate-200">
        ...
        {/* View toggle */}
        <div className="flex items-center gap-1 ml-4 bg-slate-100 rounded-md p-0.5">
          <button
            onClick={() => setViewMode("flat")}
            className={cn(
              "text-[10px] px-2 py-0.5 rounded transition-colors",
              viewMode === "flat" ? "bg-slate-700 text-slate-800" : "text-slate-500 hover:text-slate-700"
            )}
          >
            Flat
          </button>
          <button
            ...
            className={cn(
              "text-[10px] px-2 py-0.5 rounded transition-colors",
              viewMode === "threaded" ? "bg-slate-700 text-slate-800" : "text-slate-500 hover:text-slate-700"
            )}
          >
            Threaded
          </button>
              ...
              <button
                ...
                className={cn(
                  ...
                  filter === id
                    ...
                    : "border-slate-200 text-slate-600 hover:border-slate-600"
                )}
              >
        ...
        {isRunning && (
          <Badge
            variant="outline"
            className="ml-auto text-xs border-emerald-500/50 text-emerald-600 animate-pulse shrink-0"
          >
            ● LIVE
          </Badge>
        )}
        ...
        <div className="p-3 font-mono text-xs min-h-[200px]">
          ...
          {isRunning && (
            <div className="flex items-center gap-2 px-1 mt-2 mb-2 text-slate-500 min-h-[24px]">
              ...
              <span className="text-xs italic text-slate-500">Hive is working...</span>
