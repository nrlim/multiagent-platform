...
export function TerminalOutput({ logs, isRunning, className }: TerminalOutputProps) {
  ...
  return (
    <div className={cn("flex flex-col h-full", className)}>
      {/* Terminal header bar */}
      <div className="flex items-center gap-2 px-4 py-2 bg-white border-b border-slate-200 rounded-t-lg">
        ...
        {isRunning && (
          <Badge
            variant="outline"
            className="ml-auto text-xs border-emerald-500/50 text-emerald-600 animate-pulse"
          >
            ● LIVE
          </Badge>
        )}
      ...
      {/* Log content */}
      <div className="flex-1 overflow-y-auto bg-slate-50 rounded-b-lg">
        <div className="p-4 font-mono text-sm space-y-0.5 min-h-[200px]">
          {logs.length === 0 ? (
            ...
            logs.map((log) => {
              ...
              return (
                <div
                  key={log.id}
                  className="flex gap-2 group hover:bg-slate-100/30 rounded px-1 transition-colors"
                >
          ...
          {isRunning && (
            <div className="flex items-center gap-2 px-1 mt-2 mb-2 text-slate-500 min-h-[24px]">
              ...
              <span className="text-xs italic text-slate-500">Processing task...</span>
