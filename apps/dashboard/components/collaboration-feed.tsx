...
const ROLE_CFG: Record<string, {
  ...
  manager:            { bg: "bg-indigo-600",   ring: "ring-indigo-400",  text: "text-indigo-300",  icon: Brain,        presence: "Planning" },
  database_architect: { bg: "bg-sky-600",      ring: "ring-sky-400",     text: "text-sky-300",     icon: Database,     presence: "Designing Schema" },
  backend_dev:        { bg: "bg-emerald-600",  ring: "ring-emerald-400", text: "text-emerald-300", icon: Server,       presence: "Writing Code" },
  frontend_dev:       { bg: "bg-amber-500",    ring: "ring-amber-400",   text: "text-amber-300",   icon: Globe,        presence: "Building UI" },
  qa_engineer:        { bg: "bg-pink-600",     ring: "ring-pink-400",    text: "text-pink-300",    icon: FlaskConical, presence: "Reviewing" },
  devops_engineer:    { bg: "bg-slate-500",    ring: "ring-slate-400",   text: "text-slate-300",   icon: Code2,        presence: "Deploying" },
  tech_writer:        { bg: "bg-violet-500",   ring: "ring-violet-400",  text: "text-violet-300",  icon: FileText,     presence: "Documenting" },
};
const DEFAULT_ROLE = { bg: "bg-slate-700", ring: "ring-slate-500", text: "text-slate-400", icon: User, presence: "Processing" };
...
function CodeRefPill({ path }: { path: string }) {
  ...
  return (
    <span className={cn(
      ...
      isCmd
        ? "bg-slate-950 text-cyan-400 border border-slate-700"
        : "bg-slate-950 text-blue-400 border border-slate-700"
    )}>
...
function SystemRow({ text, type }: { text: string; type: string }) {
  const [Icon, color, bg] = type === "join"
    ...
    : type === "done"
      ? [CheckCircle2, "text-emerald-400", "bg-emerald-950/60 border-emerald-800/30"]
      : type === "error"
        ? [AlertTriangle, "text-red-400", "bg-red-950/60 border-red-800/30"]
        : [Zap, "text-slate-500", "bg-slate-900/40 border-slate-800/30"];
  return (
    <motion.div
...
const ThoughtBubble = memo(function ThoughtBubble({ text }: { text: string }) {
  ...
  return (
    <motion.button
      ...
      whileHover={{ x: 2 }}
    >
      <Brain className="w-3 h-3 text-indigo-400/50 shrink-0 mt-0.5 group-hover:text-indigo-400 transition-colors" />
      <AnimatePresence initial={false}>
        {expanded ? (
          <motion.p
            ...
            className="text-[10px] text-indigo-300/60 italic leading-snug overflow-hidden"
          >
            {text}
          </motion.p>
        ) : (
          <motion.span
            ...
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="text-[10px] text-indigo-400/40 italic hover:text-indigo-400/70 transition-colors"
          >
      ...
      <div className="flex-1 min-w-0">
        {isFirst && (
          <div className="flex items-baseline gap-2 mb-1.5 ml-1">
            ...
            {type === "thought" && (
              <span className="text-[10px] px-2 py-0.5 rounded-full bg-indigo-950/80 border border-indigo-500/30 text-indigo-300 shadow-sm shadow-indigo-900/20">
                thinking
              </span>
            )}
          </div>
        ...
        <div className={cn(
          ...
          type === "thought"
            ? "bg-indigo-950/30 border border-indigo-800/30 italic text-indigo-200/90"
            : "bg-slate-800/70 backdrop-blur-md border border-white/10 text-slate-100",
        )}>
...
function PresenceSidebar({ agentRoles, activeAgents, focusedAgentId, onFocus }: PresenceSidebarProps) {
  ...
  return (
    <div className="w-44 shrink-0 border-l border-white/5 flex flex-col overflow-hidden">
      {/* Header */}
          ...
          return (
            <button
              ...
              className={cn(
                ...
                isFocused ? "bg-slate-800/80" : "hover:bg-slate-800/40"
              )}
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[10px] font-semibold text-slate-300 truncate capitalize">
                  {role.replace(/_/g, " ")}
                </p>
  ...
  return (
    <div className={cn("flex h-full min-h-0 overflow-hidden", className)}>
      ...
      <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
        ...
        {/* Toolbar */}
        <div className="flex items-center gap-2 px-4 py-2 border-b border-white/5 bg-slate-900/40 shrink-0">
          <Hash className="w-3.5 h-3.5 text-slate-600" />
          <span className="text-[11px] text-slate-400 font-bold">activity-feed</span>
          {focusedAgentId && (
            ...
            {isEmpty ? (
              <div className="flex flex-col items-center justify-center h-full gap-4 pt-16">
                <motion.div
                  ...
                  className="w-12 h-12 rounded-2xl bg-slate-900/60 border border-slate-800/50 flex items-center justify-center"
                >
          ...
          <AnimatePresence>
            {typingAgents.length > 0 && (
              ...
              >
                ...
                <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-2xl bg-slate-800/50 border border-white/5">
                  <TypingDots />
